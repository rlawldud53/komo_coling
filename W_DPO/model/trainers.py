...
from model.preference_datasets import get_batch_iterator
...
def preference_loss(policy_chosen_logps: torch.FloatTensor,
    ...
    ################# included_emojis -> emoji 에 따른 loss 분석 때문에 넣었는데 사실상 필요 x ####################################
...
class BasicTrainer(object):
    ...
    "🪣", "🛁", "🪞", "🪠", "🪤", "🪒", "🪥", "🛒", "🚬", "⚰", "⚱", "🪦", "🧿", "🪔", "🪒"
        ...
        ###################################### tokenizer load하는 부분 우리 tokenizer에 맞게 수정 ########################################
        ...
        if ("canho" in config.model.name_or_path) or ("jeeyoung" in config.model.name_or_path) or ("beomi" in config.model.name_or_path):
            ...
            rank0_print(f'Loading tokenizer {base_tokenizer_name}')
        ...
        if config.loss.name != 'sft':
            get_log_token_embeddings(len(self.tokenizer))
    ...
    def train(self):
        ...
        for idx,batch in tqdm(enumerate(self.train_iterator)):
            ...
            include_emojis = [ (len(list(set(self.tokenizer.tokenize(batch['chosen'][i])) & set(self.emoji_tokens))) != 0) for i in range(len(batch['chosen'])) ] # 사용 x
            ...
            for microbatch_idx in range(self.config.gradient_accumulation_steps):
                ...
                #print(f"""{self.config.gradient_accumulation_steps,self.rank}""")
                #print(f"""{microbatch_idx} : {global_microbatch}""")
                #print(metrics)
                (loss / self.config.gradient_accumulation_steps).requires_grad_(True).backward()

                for k, v in metrics.items():
                    #if len(v) ==0 : breakpoint()
    ...
    def write_state_dict(self, step: int, state: Dict[str, torch.Tensor], metrics: Dict, filename: str, dir_name: Optional[str] = None, repo_name : Optional[str] = None):
        ...
        ########################################### checkpoint 저장하는 부분 -> hub에 로드하도록 수정 ####################################
