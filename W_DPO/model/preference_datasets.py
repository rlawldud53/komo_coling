...
################################################################## our dataset 가져오는 부분 추가 ##########################################################
def get_custom(data_path="/content/drive/MyDrive/2024_School/Coling/2nd KARMA/datasets/rag/filtering/implicit/final_Implicit_total.csv"):
    dataset = pd.read_csv(data_path)

    data = defaultdict(lambda: defaultdict(list))
    for row in tqdm.tqdm(dataset.iterrows(), desc='Processing dataset'):
        
        prompt, chosen, rejected = row[1]['input'], row[1]['response'], row[1]['response']
        responses = [chosen, rejected]
        n_responses = len(data[prompt]['responses'])
        data[prompt]['pairs'].append((n_responses, n_responses + 1))
        data[prompt]['responses'].extend(responses)
